...
public class SimpleJxtaSocketClient {
	...
	public static void main(String args[]) {
		...
		try {
			Thread.currentThread().setName(SimpleJxtaSocketClient.class.getName() + ".main()");			
			SimpleJxtaSocketClient socEx = new SimpleJxtaSocketClient();
			System.out.println("### Connections Expected: " + RUNS);
			for (int i = 1; i <= RUNS; i++) {
				System.out.println("### Socket connection #" + i);
				socEx.run();
			}
			socEx.stop();
